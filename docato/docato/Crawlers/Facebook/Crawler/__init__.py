...
from getpost import get_post_html_uft8
...
MEDIA_DIR = settings.MEDIA_ROOT #'/docato_data/media' #settings.MEDIA_ROOT
TMP_DIR = settings.TMP_DIR # '/tmp' # settings.TMP_DIR
...
class FBWithCommentsProcessor(object):
    ...
    def __call__(self, item):
        ...
        if '?' in url:
            url = url.split('?')[0]
        return self._process_post_id( url )


    # ok
    def _process_post_id(self, post_url):
        # делаем сам пост , чтобы отсечь все лишнее
        post_data, post_height , comments = get_post_html_uft8(post_url)
        post_etree = fromstring(post_data)
        #
        s = etree.tostring(post_etree, method='html')
        #
        ...
        #
        slug = 'facebook_discuss_{}'.format( re.sub('/','_',post_url) )
        ...
        with open('{0}/{1}/localhost:{2}/index.html'.format(TMP_DIR, slug, WGET_PORT), 'r') as content_file:
            ...
            content2 = mark_tokens_in_etree(content)
        ...
        with open('{0}/{1}/comments.json'.format(TMP_DIR, slug), 'w') as wholepage:
            wholepage.write(json.dumps(comments))
        # сожмём содержимое
...
#   MAIN CODE
def discussion_FB_get_byurl(url):
    proc = FBWithCommentsProcessor({})
    return proc.__call__(url)

if __name__ == '__main__':
    text = discussion_FB_get_byurl('https://www.facebook.com/groups/601430053212107/permalink/705945026093942/')
    pass